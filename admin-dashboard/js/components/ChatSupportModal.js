...
// Fixes:
//  1. clearedUserIds stored in sessionStorage → survives page reload within same tab session
//  2. Backend end-session now called with DELETE method and correct endpoint
...
// ── Persistent cleared-user storage (survives reload within browser tab session) ──
const ClearedSessions = {
    ...
    has: function (userId) {
        return this.load().has(Number(userId));
    },
...
const ChatSupportModal = ({ onClose, isAdmin = false, currentUser, selectedPatient = null }) => {
    ...
    // The in-memory ref mirrors sessionStorage so we don't hit storage on every render.
    // Both are kept in sync by ClearedSessions.add().
    ...
    // ── WebSocket: listen for new patient messages ────────────────────────────
    // Skip events for users the admin has already cleared.
    useEffect(() => {
        if (!isAdmin) return;
        ...
        const handler = function (e) {
            ...
            // If this user was cleared, ignore the event completely
            if (clearedUserIdsRef.current.has(uid)) {
                console.log('⏭️ Skipping WS event for cleared userId:', uid);
                return;
            }

            // Otherwise refresh the sidebar list
            loadConversations(true);

            // If the admin has this user open, also refresh messages
            const conv = activeConvRef.current;
            if (conv && Number(conv.userId) === uid) {
    ...
    // Always filters cleared users FIRST using the persistent set.
    const loadConversations = async function (silent) {
        ...
        try {
            ...
            if (!res.ok) { console.error('loadConversations HTTP error', res.status); return; }
            const data = await res.json();
            if (!data.success || !data.chats) return;

            // Re-read from storage on every load so reloads pick up the persisted set
            ...
            const mapped = data.chats
                ...
                // ── KEY FIX: strip cleared rows on every load, including reloads ──
    ...
    const loadMessagesForUser = async function (userId, silent) {
        if (!userId) return;
        // Never load messages for a cleared user
    ...
    // ── End session (admin) ───────────────────────────────────────────────────
    // Order of operations:
    //  1. Persist userId to sessionStorage IMMEDIATELY (survives reload)
    //  2. Update in-memory ref
    //  3. Wipe local UI
    //  4. Call backend (best-effort — UI is already clean regardless)
    const endSessionAndClear = async function (userId, patientName) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const deleted  = data.deletedMessages  ?? '?';
                const resolved = data.resolvedTickets  ?? '?';
                ...
                console.log(`✅ Backend: deleted ${deleted} messages, resolved ${resolved} tickets for user ${uid}`);
            } else if (res.status === 404) {
                console.warn('⚠️ /api/support/admin/end-session returned 404 — deploy the endpoint on Railway');
                window.showNotificationAlert && window.showNotificationAlert(
                    'Cleared from view ✅ (backend endpoint missing — redeploy to also wipe patient messages)'
                );
            } else if (res.status === 403) {
                console.error('403 — check SecurityConfig allows POST /api/support/admin/end-session/**');
                window.showNotificationAlert && window.showNotificationAlert(
                    'Cleared from view ✅ (403 — add endpoint to SecurityConfig permit list)'
                );
            } else {
                let msg = 'Server error ' + res.status;
                try { const d = await res.json(); msg = d.message || msg; } catch (e2) {}
                console.error('end-session backend error:', msg);
                window.showNotificationAlert && window.showNotificationAlert(
                    'Cleared from view ✅ (backend error: ' + msg + ')'
                );
            }
        } catch (netErr) {
            console.error('end-session network error:', netErr.message);
            window.showNotificationAlert && window.showNotificationAlert(
    ...
    const _applyLocalClear = function (userId) {
        ...
        setConversations(function (prev) {
            return prev.filter(function (c) { return Number(c.userId) !== Number(userId); });
        });
