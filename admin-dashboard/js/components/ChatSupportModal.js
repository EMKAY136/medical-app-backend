...
// Fixed: removed useCallback (causes "c is not a function" in in-browser Babel),
//        fixed setPrevHumanIds setState closure bug, simplified alert handling.
...
const ChatSupportModal = ({ onClose, isAdmin = false, currentUser, selectedPatient = null }) => {
  ...
  const [newRequestAlert,    setNewRequestAlert]    = useState(null); // { name }
  ...
  // Store previous human-request IDs without using state (avoids closure issues)
  ...
  const loadConversations = async (silent) => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('loadConversations error', res.status);
        return;
      }
      const data = await res.json();
      if (!data.success || !data.chats) return;

      const mapped = data.chats.map(function(chat) {
      ...
      var humanOnes = mapped.filter(function(c) { return c.isHumanRequest; });
      setHumanRequestCount(humanOnes.length);

      // Detect NEW human requests using ref (avoids setState closure bug)
      var brandNew = humanOnes.filter(function(c) { return !prevHumanIdsRef.current.has(c.userId); });
      if (brandNew.length > 0) {
        showNewRequestAlert(brandNew[0].patientName);
      }
      var newSet = new Set(humanOnes.map(function(c) { return c.userId; }));
      prevHumanIdsRef.current = newSet;
  ...
  // Flash a new-agent-request alert
  ...
  // ── Open a patient conversation (from sidebar) ─────────────────────────────
  ...
  const endSessionAndClear = async function(userId, patientName) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        window.showNotificationAlert && window.showNotificationAlert('Session ended — chat cleared ✅');
      } else if (res.status === 404) {
        // Endpoint not deployed yet — clear locally anyway
        clearSessionLocally(userId);
        window.showNotificationAlert && window.showNotificationAlert('Cleared locally ✅ (backend endpoint not found — redeploy needed)');
      } else if (res.status === 403) {
        setSessionEndError('403 — Check SecurityConfig allows POST /api/support/admin/** for ADMIN role.');
        clearSessionLocally(userId);
      } else {
        var errData = {};
      ...
      clearSessionLocally(userId);
      window.showNotificationAlert && window.showNotificationAlert('Cleared locally ✅ (network error)');
    } finally {
      setEndingSession(false);
    }
  };

  const clearSessionLocally = function(userId) {
    setMessages([]);
    setConversations(function(prev) { return prev.filter(function(c) { return c.userId !== userId; }); });
    setActiveConversation(null);
    activeConvRef.current = null;
    setSessionEndError(null);
    var newSet = new Set(prevHumanIdsRef.current);
  ...
  const sendMessage = async function() {
    ...
    try {
      ...
      if (data.success) {
        ...
        var activeConv = activeConvRef.current;
        setTimeout(function() {
  ...
  const isMyMessage = function(msg) {
    var t = (msg.senderType || '').toLowerCase();
  ...
  var filteredConversations = conversations.filter(function(c) {
                  ...
                  : filteredConversations.map(function(conv) {
                      return React.createElement('div', {
                        ...
                        style: {
                          ...
                          cursor:          'pointer',
                          borderBottom:    '1px solid #e5e7eb',
                          backgroundColor: activeConversation && activeConversation.id === conv.id
                            ? '#dbeafe'
            ...
            activeConversation ? React.createElement(React.Fragment, null,
              ...
              React.createElement('div', {
                ...
                isAdmin && React.createElement('div', { style: { display: 'flex', gap: '8px', alignItems: 'center' } },
                  ...
                  }, endingSession ? 'Ending...' : '🗑 End & Clear')
                )
              ),
                ...
                loading && messages.length === 0
                  ...
                  : messages.length === 0
                    ...
                    : messages.map(function(msg) {
                        var mine    = isMyMessage(msg);
                        var bgColor = getBubbleBg(msg);
                        var isBot   = (msg.senderType || '').toLowerCase() === 'bot';
                        return React.createElement('div', {
                          key:   msg.id,
