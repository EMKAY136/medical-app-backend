...
const MedicalAdminDashboard = () => {
    ...
    // ── Derived from appointments — single source of truth ───────────────────
    // Refund requests: any appointment where patient requested a refund
    const refundRequestsList = appointments.filter(a =>
        (a.refundStatus || '').toUpperCase() === 'REQUESTED' ||
        a.refundRequested === true ||
        (a.paymentStatus || '').toUpperCase() === 'REFUND_REQUESTED'
    );

    // Reschedule requests: any appointment where patient asked to reschedule
    const rescheduleRequestsList = appointments.filter(a =>
    ...
    useEffect(() => {
        if (isAuthenticated && window.AdminWebSocketClient) {
            ...
            // Immediately bump badge when human-agent request arrives via WebSocket
            // so admin sees the badge without waiting for the 10s poll cycle
            const handleChatEvent = (e) => {
                const d   = e.detail || {};
                const msg = (d.msg || d.message || d.subject || '').toLowerCase();
                const sub = (d.subject  || '').toLowerCase();
                const cat = (d.category || '').toLowerCase();
                const isHumanReq =
                    msg.includes('human') || msg.includes('agent') ||
                    sub.includes('human support') || cat.includes('human support');
                if (isHumanReq) {
                    setChatUnreadCount(prev => {
                        // Only update if support chat is closed
                        const chatOpen = document.querySelector('.chat-support-modal-open');
                        return chatOpen ? prev : prev + 1;
                    });
                    // Also refresh count from server to get accurate total
                    loadChatUnreadCount();
                }
            };

            window.addEventListener('newPatientChatMessage', handleChatEvent);
            window.addEventListener('newSupportTicket',      handleChatEvent);

            return () => {
                wsClient.disconnect();
                window.removeEventListener('newPatientChatMessage', handleChatEvent);
                window.removeEventListener('newSupportTicket',      handleChatEvent);
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.success && data.chats) {
                    // Count chats where patient explicitly requested a human agent
                    // Backend returns subject='Human Support Requested' / category='Human Support'
                    const humanRequests = data.chats.filter(c =>
                        (c.subject   || '').toLowerCase().includes('human support') ||
                        (c.category  || '').toLowerCase().includes('human support') ||
                        (c.subject   || '').toLowerCase().includes('human agent') ||
                        c.conversationType === 'NEEDS_FIRST_RESPONSE'
                    ).length;
                    // Only update if chat panel is closed — don't flash badge while admin is reading
                    if (!showSupportChat) setChatUnreadCount(humanRequests);
                }
            }
        } catch { /* silent */ }
    };
    ...
    const loadStats = () => {
        try {
            ...
            // ── Derived directly from appointments — same logic as refundRequestsList ──
            const refundCount = appointments.filter(a =>
                (a.refundStatus || '').toUpperCase() === 'REQUESTED' ||
                a.refundRequested === true ||
                (a.paymentStatus || '').toUpperCase() === 'REFUND_REQUESTED'
            ).length;
    ...
    // ── Guards ────────────────────────────────────────────────────────────────
    if (validatingToken) return <div className="loading"><div className="spinner"></div><p>Verifying session...</p></div>;
    if (!isAuthenticated) return React.createElement(Login, { onLoginSuccess: handleLoginSuccess });
    if (loading && patients.length === 0) return <div className="loading"><div className="spinner"></div><p>Loading dashboard...</p></div>;
    ...
    return (
        <div className="dashboard" style={{ display: 'flex', minHeight: '100vh' }}>
            ...
            <div className="main-content" style={{ flex: 1, overflowY: 'auto' }}>
                ...
                {currentView === 'dashboard' && (
                    <div>
                        ...
                        {/* Refund requests banner — only shown when there are pending refunds */}
                        {stats.refundRequests > 0 && (
                            <div style={{ margin: '0 20px 20px', padding: '14px 18px', background: '#f3e8ff', border: '2px solid #a78bfa', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <button onClick={() => setCurrentView('appointments')}
                                    style={{ padding: '8px 16px', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '13px', whiteSpace: 'nowrap' }}>
                                    Review →
                                </button>
                            </div>
                        )}
                ...
                {currentView === 'appointments' && React.createElement(AppointmentsView, {
                    ...
                    // ✅ Pass derived lists so AppointmentsView uses the SAME data source
                    refundRequestsList,
            ...
            {showSupportChat && window.ChatSupportModal && React.createElement(window.ChatSupportModal, {
                onClose: () => { setShowSupportChat(false); setSupportChatPatient(null); setChatUnreadCount(0); setTimeout(loadChatUnreadCount, 1000); },
                isAdmin: true,
            ...
            <style>{`
                ...
                @keyframes btn-pulse {
                    ...
                    50% { box-shadow: 0 4px 24px rgba(239,68,68,0.9), 0 0 0 8px rgba(239,68,68,0.15); }
                }
            `}</style>
            {!showSupportChat && (
                <button
                    onClick={() => { setSupportChatPatient(null); setShowSupportChat(true); setChatUnreadCount(0); }}
                    title={chatUnreadCount > 0 ? `${chatUnreadCount} patient(s) need live support!` : 'Open Patient Support Chat'}
                    style={{
                        ...
                        animation: chatUnreadCount > 0 ? 'btn-pulse 1.5s ease-in-out infinite' : 'none',
                    }}
                ...
                >
                    ...
                    {chatUnreadCount > 0 && (
                        <span style={{
                            ...
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            padding: '0 5px', border: '2px solid #ef4444', zIndex: 1000,
