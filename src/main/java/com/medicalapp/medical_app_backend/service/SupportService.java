...
public class SupportService {
    ...
    // Adds a system message; does NOT delete history so admin can still read it.
            ...
            if (query == null || query.trim().isEmpty()) {
                ...
                for (User user : allUsers) {
                    if (user.getRole() == User.Role.PATIENT) {
                        ...
                        if (fullName.contains(searchTerm) || user.getEmail().toLowerCase().contains(searchTerm)) {
    ...
    // ── Get chat by user ID (admin view) ──────────────────────────────────────
    ...
    public Map<String, Object> getDebugInfo(UserDetails agentDetails) {
        ...
        try {
            ...
            List<User> allUsers      = userRepository.findAll();
            List<ChatMessage> allMsg = chatMessageRepository.findAll();

            Set<Long> usersWithTickets  = supportTicketRepository.findAll().stream().map(t -> t.getUser().getId()).collect(Collectors.toSet());
            Set<Long> usersWithMessages = allMsg.stream().map(m -> m.getUser().getId()).collect(Collectors.toSet());
            Set<Long> chatOnlyUsers     = new HashSet<>(usersWithMessages);
            chatOnlyUsers.removeAll(usersWithTickets);

            response.put("success", true);
            response.put("database", Map.of("totalUsers", userRepository.count(), "totalTickets", supportTicketRepository.count(), "totalMessages", chatMessageRepository.count()));
            ...
            response.put("messages", Map.of(
                "fromUsers",  allMsg.stream().filter(m -> m.getSenderType() == ChatMessage.SenderType.USER).count(),
                "fromBots",   allMsg.stream().filter(m -> m.getSenderType() == ChatMessage.SenderType.BOT).count(),
                "fromAgents", allMsg.stream().filter(m -> m.getSenderType() == ChatMessage.SenderType.SUPPORT_AGENT).count()
            ));
    ...
    // ── Get chat history (patient's own) ──────────────────────────────────────
    ...
    public Map<String, Object> getActiveChats(UserDetails agentDetails) {
        ...
        try {
            ...
            latestMessagePerUser.values().forEach(m -> activeChats.add(createChatInfoFromMessage(m, "CHAT_ONLY")));
    ...
    private Map<String, Object> createPatientInfo(User user) {
        ...
        long openCount = userTickets.stream()
